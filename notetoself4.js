...
function getStickiesArray() {
	...
	if (!stickiesArray) {
		...
		OscillatorNode.log(stickiesArray);
	} else {
...
function remove_list() {
	list_parent = document.getElementById('dami');
	list_child = list_parent.childNodes;
	list_children = list_parent.children;
	console.log(list_children + list_children.length);
	for (var i = list_child.length-1; i >=0; i--) {
		
		list_parent.removeChild(list_child[i]);
	}
}
